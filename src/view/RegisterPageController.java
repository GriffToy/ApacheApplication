...
public class RegisterPageController {
	...
	/**
	 * Confirms with the user that the registration details will be lost, before
	 * displaying the userSelectPage.
	 * 
	 * @author Griffin Toyoda
	 */
	@FXML
	private void backButtonClicked() {
		if (mainApp != null) {
			// Popup warning: go back to login page if user confirms they want
			// to go back.
	...
	/**
	 * If all fields are filled out correctly, according to verifyFields(), a
	 * new user is created and added to the user map.
	 * 
	 * @author Griffin Toyoda
	 */
	@FXML
	private void submitButtonClicked() {
		...
		if (verifyFields()) {
			...
			try {
				...
				String query = "INSERT INTO User VALUES( " + newUser.getAttendeeID() + ", '" + newUser.getLastName()
						+ "', '" + newUser.getFirstName() + "', '" + newUser.getUsername() + "', '"
						+ newUser.getPassword() + "', '" + newUser.getEmailAddress() + "', " + newUser.getPhoneNumber()
						+ ", '";
				if (newUser.getUserType() == UserType.ATTENDEE)
					query += "attendee')";
				else
					query += "judge')";
...
/*	private boolean validNewUser() {
		...
		String query = "select * from User where attendeeEmail = ? " + "or attendeeUserName = ?";
		try {
			...
			if (result.next()) {
				warningText.setText("Duplicate user! Error");
				return false;
			}

			pst.close();
		} catch (Exception e) {
			warningText.setText("Error Connection!" + e.getMessage());
		}
	...
	/**
	 * Confirms with the user that the registration details will be lost, before
	 * displaying the loginPage.
	 * 
	 * @author Griffin Toyoda
	 */
	@FXML
	private void cancelButtonClicked() {
		// Popup warning, go back to login page if user confirms they want to go
		// back.
		Alert alert = new Alert(AlertType.CONFIRMATION);
		alert.setTitle("Confirmation Dialog");
	 ...
	 * 
	 * @author Griffin Toyoda
	 * @return True if fields in the registration page are valid, false
	 *         otherwise.
	 */
